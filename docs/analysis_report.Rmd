...
We trimmed sequenced reads using CutAdapt to remove adapter sequences and low quality bases. We then aligned reads to the provided RefSeq reference NC_045512.2 using BWA. Subsequently, the resulting bam files have been used for variant calling with both a widely used somatic variant caller, LoFreq, as well as with a variant caller specifically developed for amplicon based sequencing and viral evolution analysis, iVar.
...
We provide a full list of variant calls by LoFreq, filtered by passing criteria requiring depth of 100 at the position and a variant allele fraction above 5%.

```{r}
#library(DT)
#vartable <- read.csv(params$varianttable)
#lofreqvars <- filter(vartable, Basic_Pass=="True", Caller=="lofreq")
#lofreqvars <- select(lofreqvars, c("Sample","Region","Position","Ref","Alt","Depth","Proportion","Gene"))
#datatable(lofreqvars, caption="Table of Passing LoFreq Variant Calls")
#ivarvars <- filter(vartable, Basic_Pass=="True", Caller=="ivar")
#ivarvars <- select(ivarvars, c("Sample","Region","Position","Ref","Alt","Depth","Proportion","Gene"))
#datatable(ivarvars, caption="Table of Passing iVar Variant Calls")

library(tidyverse)
library(pander)
vartable <- read.csv(params$varianttable)
lofreqvars <- filter(vartable, Basic_Pass=="True", Caller=="lofreq")
lofreqvars <- select(lofreqvars, c("Sample","Position","Ref","Alt","Depth","Proportion","Gene"))
pander(lofreqvars)
```
...
library(pander)
ivarvars <- filter(vartable, Basic_Pass=="True", Caller=="ivar")
ivarvars <- select(ivarvars, c("Sample","Position","Ref","Alt","Depth","Proportion","Gene"))
pander(ivarvars)
```
...
ggplot(data=depthgathered)+
  ...
  ggtitle("Depth of Coverage Across Reference NC_045512.2")
```
...
- allele-fraction threshold = 0.05
- minimum read depth = 100

...
We provide a full list of variant calls by LoFreq, filtered by passing criteria requiring alt supporting read depth of 100 at the position and a variant allele fraction above 1%.
...
lofreqvars <- filter(vartable, Basic_Pass=="True", Caller=="lofreq")
lofreqvars <- lofreqvars[,c("Sample","Position","Ref","Alt","Ref_Reads","Alt_Reads","Proportion","Gene")]
pander(lofreqvars)
```
...
vartable <- read.csv(params$varianttable)
ivarvars <- filter(vartable, Basic_Pass=="True", Caller=="ivar")
ivarvars <- ivarvars[,c("Sample","Position","Ref","Alt","Ref_Reads","Alt_Reads","Proportion","Gene")]
pander(ivarvars)
```

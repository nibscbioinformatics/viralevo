...
params.adapter_forward = 'https://raw.githubusercontent.com/nibscbioinformatics/testdata/master/covid19/nexteraPE.fasta'
params.adapter_reverse = 'https://raw.githubusercontent.com/nibscbioinformatics/testdata/master/covid19/nexteraPE.fasta'
...
def helpMessage() {
    ...
    References:                       If not specified in the configuration file or you wish to overwrite any of the references
      --adapter_forward [file]                Path to fasta for forward adapter sequences to be trimmed
	  --adapter_reverse [file]                Path to fasta for reverse adapter sequences to be trimmed
    ...
    Other options:
      ...
      --trim_reverse_read             Number of bases to remove from the reverse read (Default: 0)
...
ch_adapter_forward = params.adapter_forward ? Channel.value(file(params.adapter_forward)) : "null"
ch_adapter_reverse = params.adapter_reverse ? Channel.value(file(params.adapter_reverse)) : "null"
...
process docutadapt {
  ...
  file(adapterfile_forward) from ch_adapter_forward
  file(adapterfile_reverse) from ch_adapter_reverse
  ...
  script:
  """
  cutadapt -a file:${adapterfile_forward} -A file:${adapterfile_reverse} -g file:${adapterfile_forward} -G file:${adapterfile_reverse} -o ${sampleprefix}.R1.trimmed.fastq.gz -p ${sampleprefix}.R2.trimmed.fastq.gz $forward $reverse -q 30,30 --minimum-length 50 --times 40 -e 0.1 --max-n 0 -U ${params.trim_reverse_read} > ${sampleprefix}.trim.out 2> ${sampleprefix}.trim.err
  """
}

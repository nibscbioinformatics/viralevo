...
```{r}
library(pander)
vartable <- read.csv(params$varianttable, header=TRUE, row.names=NULL)
#vartable <- distinct(vartable)
vartable$Ref <- as.character(vartable$Ref)
vartable$Alt <- as.character(vartable$Alt)
pander(filter(vartable, Basic_Pass=="True"))

#kable(filter(vartable, Caller=="lofreq")[,3:9]) %>%
#  kable_styling()
```
